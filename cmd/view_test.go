...
// TestViewFullStatic_QueuedStack verifies that the static full-view output shows
// a "queued" separator and the ◎ icon for a queued branch.
func TestViewFullStatic_QueuedStack(t *testing.T) {
	s := stack.Stack{
		Trunk: stack.BranchRef{Branch: "main"},
		Branches: []stack.BranchRef{
			{Branch: "b1", PullRequest: &stack.PullRequestRef{Number: 1}},
	...
	restore := git.SetOps(&git.MockOps{
		...
		IsAncestorFn:    func(string, string) (bool, error) { return true, nil },
		RevParseFn:      func(ref string) (string, error) { return "sha-" + ref, nil },
		LogFn:           func(string, int) ([]git.CommitInfo, error) { return nil, nil },
	})
	defer restore()
...
// TestViewFullStatic_MixedQueuedAndMerged verifies that the static full-view
// output shows both "queued" and "merged" separators in the correct order.
func TestViewFullStatic_MixedQueuedAndMerged(t *testing.T) {
	s := stack.Stack{
		Trunk: stack.BranchRef{Branch: "main"},
		Branches: []stack.BranchRef{
			{Branch: "b1", PullRequest: &stack.PullRequestRef{Number: 1, Merged: true}},
	...
	// Render order top-to-bottom: b3 (active) → queued separator → b2 → merged separator → b1
	queuedIdx := indexOf(output, "queued")
	mergedIdx := indexOf(output, "merged")
	assert.Less(t, queuedIdx, mergedIdx, "queued separator should appear before merged separator")
}
